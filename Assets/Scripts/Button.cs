...
public class Button : MonoBehaviour                                     //변수 설정
{
    ...
    public GameObject square6;                                         //7번 줄에서 설명한 회색 네모 UI들 중 왼쪽에서 여섯번째의 것

    public GameObject redUI;                                            //빨간 불 UI
    public GameObject black1UI;                                         //초록 불을 가리고 있는 검은색 UI

    public GameObject SquareButton;                                    //PUSH가 적혀있는 네모난 모양의 버튼 UI
    ...
    public void OnClickSquareButton()
    {
        ...
        if (isPuzzleSolved)                                               //퍼즐 해결 조건을 달성하면
        {
            redUI.SetActive(false);                                      //빨간 불 UI 비활성화
            black1UI.SetActive(false);
            Debug.Log("퍼즐이 해결되었습니다");
        }
    }
}